...
const mavenKeys = Object.keys(mavenFieldsSchema.describe().children)
...
async function configure(params: ConfigureModuleParams<MavenContainerModule>) {
  ...
  const mavenFields = validateWithPath({
    config: pick(params.moduleConfig.spec, mavenKeys),
  ...
  containerConfig.spec = <ContainerModuleSpec>omit(moduleConfig.spec, mavenKeys)
